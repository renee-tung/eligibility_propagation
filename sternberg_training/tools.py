...
def generate_target_continuous_sternberg(settings, label):
    ...
    z = np.zeros((1, T))
    if label == 1:
        z[0, 10+probe_T:probe_T+10+stim_dur] = 1 # add 10 to probe_T 
        # z[0, probe_T:] = 1 # don't add 10 to probe_T 
    elif label == -1:
        z[0, 10+probe_T:probe_T+10+stim_dur] = -1
        # z[0, probe_T:] = -1

    return np.squeeze(z)

def generate_sternberg_task_data(batch_size, settings):
    inputs = []
    targets = []
    ...
    for b in range(batch_size):
        ...
        labels[b] = label
    inputs = np.array(inputs) # shape: (batch_size, n_channels, T)
    targets = np.array(targets) # shape: (batch_size, T)
    # # output tf tensors
...
def update_plot(plot_result_values, ax_list, plot_traces=False, batch=0, n_max_neuron_per_raster=10, title=None,
    ...
    line_base, = ax.plot(presentation_steps, np.ones_like(presentation_steps) * 0, '--', color='grey',
                         label='decision threshold', alpha=0.7)
